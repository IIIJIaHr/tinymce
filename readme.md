...
Для того что бы работал плагин файловый менеджер, нужно в папку js/plugins(появится после билда) положить его исходники из OldPlugins. 

Еще я поменял имя на просто plugin.js. Если билдить для prod возможно tinymce уже будет искать plugin.min.js. Тогда нужно будет вернуть.